...
export default function Image({
  ...
  // Always initialize isLoading to true so SSR and initial client hydration match 100%
    ...
    <div className={`${baseWrapper} ${layoutWrapper} ${className}`}>
      {(hasNoImage || isError) && (
        <div className="absolute inset-0 z-20 flex flex-col items-center justify-center bg-gray-100 text-gray-400">
          ...
          <span className="text-xs font-medium">{hasNoImage ? "No image" : "Failed to load"}</span>
        </div>
      )}

      {isLoading && !isError && !hasNoImage && (
        <div className="absolute inset-0 z-10 bg-gray-200 pointer-events-none before:absolute before:inset-0 before:-translate-x-full before:animate-[shimmer_1.5s_infinite] before:bg-gradient-to-r before:from-transparent before:via-white/60 before:to-transparent" />
      )}
