...
export default function AboutStrengths({ data }: AboutStrengthsProps) {
  ...
  return (
    <section className="w-full relative overflow-hidden bg-primary px-4 2xl:px-[240px] py-[80px] font-sans flex justify-center">
      ...
      <div className="w-full  flex flex-col lg:flex-row gap-[60px] lg:gap-[100px] relative z-10">
        <div className="w-full lg:w-[500px] shrink-0">
          <div className="sticky flex flex-col items-start gap-5">
            ...
            <h2 className="text-[36px] md:text-[46px] font-bold leading-[120%] text-[#fdfdfd] tracking-tight">
              Keunggulan Kompetitif Kami
            </h2>

            <p className="text-[16px] leading-[180%] font-medium text-[#fdfdfd]/80">
        ...
        <div className="flex-1 flex flex-col">
          {data.strengths.map((item, index) => {
            ...
            return (
              <div
                key={item.id || index}
                className={`flex flex-col md:flex-row items-start gap-6 md:gap-10 py-[40px] group transition-all duration-300 ${
                  !isLast ? "border-b border-[#fdfdfd]/15" : ""
                }`}
              >
                <div className="text-[56px] md:text-[72px] font-black leading-none text-[#fdfdfd]/20 group-hover:text-[#7eb2fc] transition-colors duration-500 shrink-0 font-mono tracking-tighter">
                  {displayNumber}
                </div>

                <div className="flex flex-col gap-3 mt-2">
                  <h3 className="text-[22px] md:text-[26px] font-bold leading-[130%] text-[#fdfdfd] group-hover:translate-x-2 transition-transform duration-300">
                    {item.title}
                  </h3>

                  {item.description && (
                    <p className="text-[15px] md:text-[16px] font-medium leading-[180%] text-[#fdfdfd]/70 group-hover:text-[#fdfdfd]/90 transition-colors duration-300">
                      {item.description}
                    </p>
                  )}
                </div>
